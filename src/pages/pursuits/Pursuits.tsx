...
const PursuitsPage: React.FC = () => {
  ...
  const columnsData = useMemo(() => {
    ...
    filteredProjects.forEach((p) => {
      ...
      (result[norm] ?? result[ProjectStatus.OPEN]).push(p);
    });
    return result;
  }, [filteredProjects]);
  ...
  const fetchAllDimensionStatuses = async (projectId: string) =>
    await Promise.all(dimOrder.map((d) => getDimensionStatus(projectId, d)));
  ...
  const handleDragEnd = async (e: DragEndEvent) => {
    ...
    if (
      ...
      try {
        ...
        if (newStatus === ProjectStatus.EXECUTION && someBad(dims)) {
          allowed = false;
          toast.info(
            "Cannot move to EXECUTION: at least one dimension is Bad."
          );
        }
        if (newStatus === ProjectStatus.CANCELLED && noneBad(dims)) {
          allowed = false;
          toast.info(
            "Cannot move to CANCELLED: all dimensions are Good/Warning."
          );
        }
      } catch (err) {
        ...
        allowed = false;
        toast.error(
          "Could not validate dimensions. Please try again later."
        );
      }
    }
    if (!allowed) return;
    ...
    try {
      ...
      toast.error("Error updating project: " + (err as any).message);
    }
  };
